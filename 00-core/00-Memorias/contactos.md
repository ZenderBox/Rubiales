# Contactos clave — Proyecto Avícola Rubiales

## Entidades públicas

### ICA Mosquera (Tibaitatá)
- Correo: `gerencia.cmarca@ica.gov.co`
- Contexto: papá ya habló verbalmente, le dijeron que enviara correo formal.
- Pregunta abierta: ¿pueden tramitar desde Mosquera y no Villavicencio?

### Cormacarena
- Tipo: Corporación autónoma regional (autoridad ambiental Meta).
- Trámite crítico: permiso de uso de agua. **Sin esto no hay ICA, no hay proyecto.**
- Papá va a viajar para iniciar trámite.
- Pendiente: documentar requisitos exactos (caudal, fuente, uso).

### Agrosavia — semilla plátano
- Contacto: **Rodrigo Martínez**, jefe línea Dominico Hartón.
- Correo: `ramartinez@agrosavia.co`
- Rol: nos consigue la semilla (plántula in vitro o colino) en Colombia.

## Proveedores de gallinas

### COLAVES
- Sede: Girón, Santander
- Dirección: Cl. 58 #8-65, Girón, Santander
- Tel: +57 (607) 624 5064
- Email: `recepcion@colaves.com`
- Asesora zona Meta/Cundinamarca/Boyacá: **Olga Lucía Sánchez** — Cel `317 639 9473` — Email `Centro@colaves.com`
- Web: www.colaves.com
- Líneas: ISA Brown, Shaver Black
- ⚠️ Vende pollitas de **1 día**, no de 16 semanas. Hay que preguntar si tienen levante hasta semana 16 o aliado.

### SAN MARINO
- Misma zona (Girón) — visitar mismo viaje que Colaves.
- Papá va a llamar al vendedor para cotizar.
- **Pendiente:** pedirle a papá el contacto del vendedor y guardarlo acá.

### TOSCANA
- Solo por llamada / WhatsApp — **NO acepta visitas**.
- Precio referencia: ~$26.000 por unidad (negociable según volumen).

## Cliente

### Duflo
- Comprador confirmado: compra TODO lo que se produzca (184.000 huevos/mes + plátano + pollo).
- Pendiente: visita presencial → correo formal de seguimiento (T05).
- Pregunta a explorar: ¿conocen ingenieros civiles de confianza en la zona?

## Construcción

### Moreno Vargas — Obras Civiles
- Hace obras civiles de mayor escala.
- Papá considera que **no es el fit** para el galpón.
- Buscar ingenieros de la zona como alternativa.

### Constructor galpón (pendiente identificar nombre)
- Papá ya empezó a cotizar con un ingeniero de la zona.
- **Pendiente:** pedirle a papá el nombre y datos del contacto.

## Equipo interno

### Sociedad Agropecuaria Miramar Jaime Acuña e Hijos S en C
- **NIT:** 900.798.798-8
- **Matrícula mercantil:** 02526088 · Cámara de Comercio de Bogotá
- **Email registrado en Cámara:** `JAIMEHA13@HOTMAIL.COM`
- **Dirección fiscal:** Cra 7 #12B-84, Oficina 904, Bogotá D.C.
- Datos legales completos en `datos-sociedad.md`.

Socios y roles:
- Juan Acuña — 64% — Estratega / Finanzas (en Miami)
- Alejandro Acuña — 33%
- **Jaime Acuña Ocampo** (papá) — Socio gestor / Representante Legal — C.C. 17.310.188 — operación en campo

### Contadora
- Pendiente: registrar nombre y datos de contacto.
